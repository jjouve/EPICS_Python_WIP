...
To create an instance of the epicsMotor class pass the record name
(without any record field) to the class constructor.  For example:
   m=epicsMotor('13BMD:m38')

Methods:
   move(position, [relative=1], [dial=1], [step=1], [ignore_limits=1])
   wait([start=1], [stop=1], [poll=.01], [ignore_limits=1])
   stop()
   get_position([dial=1], [step=1], [readback=1])
   set_position([dial=1], [step=1], [readback=1])
   check_limits()
...
"""
   ...
   def __init__(self, name):
      self.pvs = {'val' : epicsPV(name+'.VAL',  wait=0), 
                  'dval': epicsPV(name+'.DVAL', wait=0),
                  'rval': epicsPV(name+'.RVAL', wait=0),
                  'rlv' : epicsPV(name+'.RLV',  wait=0),
                  'rbv' : epicsPV(name+'.RBV',  wait=0),
                  'drbv': epicsPV(name+'.DRBV', wait=0),
                  'rrbv': epicsPV(name+'.RRBV', wait=0),
                  'dmov': epicsPV(name+'.DMOV', wait=0),
                  'stop': epicsPV(name+'.STOP', wait=0),
                  'velo': epicsPV(name+'.VELO', wait=0),
                  'vbas': epicsPV(name+'.VBAS', wait=0),
                  'accl': epicsPV(name+'.ACCL', wait=0),
                  'desc': epicsPV(name+'.DESC', wait=0),
                  'mres': epicsPV(name+'.MRES', wait=0),
                  'hlm':  epicsPV(name+'.HLM',  wait=0),
                  'llm':  epicsPV(name+'.LLM',  wait=0),
                  'dhlm': epicsPV(name+'.DHLM', wait=0),
                  'dllm': epicsPV(name+'.DLLM', wait=0),
                  'bdst': epicsPV(name+'.BDST', wait=0),
                  'set':  epicsPV(name+'.SET',  wait=0),
                  'lvio': epicsPV(name+'.LVIO', wait=0),
                  'lls':  epicsPV(name+'.LLS',  wait=0),
                  'hls':  epicsPV(name+'.HLS',  wait=0),
                  'off':  epicsPV(name+'.OFF',  wait=0)
                  }
   ...
   def move(self, value, relative=0, dial=0, step=0, ignore_limits=0):
      """
      move(value)
         Description: Moves a motor to an absolute position or relative to the
                      current position in user, dial or step coordinates.
         Parameters:
            value:    The absolute position or relative amount of the move
         Keywords:
            relative: Set relative=1 to move relative to current position.
                      The default is an absolute move.
            dial:     Set dial=1 if "value" is in dial coordinates.
                      The default is user coordinates.
            step:     Set step=1 if "value" is in steps.
                      The default is user coordinates.
            ignore_limits: Set ignore_limits=1 to suppress raising exceptions
   ...
   def stop(self):
      """
      stop()
         Description: Immediately stops a motor from moving by writing 1 to
                      the .STOP field.
         Examples:
            m=epicsMotor('13BMD:m38')
            m.move(10)          # Move to position 10 in user coordinates
   ...
   def get_position(self, dial=0, readback=0, step=0):
      """
      p = get_position()
         Description: Returns the target or readback motor position in user, 
                      dial or step coordinates.
         Keywords:
            readback: Set readback=1 to return the readback position in the
                      desired coordinate system.  The default is to return the
                      target position of the motor.
            dial:     Set dial=1 to return the position in dial coordinates.
                      The default is user coordinates.
            step:     Set step=1 to return the position in steps.
                      The default is user coordinates.
         Notes:
            The "step" and "dial" keywords are mutually exclusive.
   ...
   def set_position(self, position, dial=0, step=0):
      """
      set_position()
         Description: Sets the motor position in user, dial or step coordinates.
         Parameters:
            position: The new motor position
         Keywords:
            dial:     Set dial=1 to set the position in dial coordinates.
                      The default is user coordinates.
            step:     Set step=1 to set the position in steps.
                      The default is user coordinates.
         Notes:
            The "step" and "dial" keywords are mutually exclusive.
         Examples:
   ...
   def wait(self, start=0, stop=0, poll=0.01, ignore_limits=0):
      """
      wait()
         Description: Waits for the motor to start moving and/or stop moving.
         Keywords:
            start:  Set start=1 to wait for the motor to start moving.
            stop:   Set stop=1 to wait for the motor to stop moving.
            poll:   Set this keyword to the time to wait between reading the
                    .DMOV field of the record to see if the motor is moving.
                    The default is 0.01 seconds.
            ignore_limits: Set ignore_limits=1 to suppress raising an exception
                    if a soft or hard limit is detected
         Notes:
